...
// const clipboardIcon = (
//   <svg viewBox="0 0 25 24" stroke="currentColor" strokeWidth={2} fill="none" strokeLinecap="round" strokeLinejoin="round">
...
class App extends Component {
  ...
  themeListRenderer() {
    ...
    return themeList.map((t, k) => <button key={k} data-theme={t} onKeyDown={this.keyboardNavigation} className={`theme-button bg-${t}-500${theme === t ? ' is-active' : ''}`} onClick={this.changeTheme}></button>)
  }
      ...
      <div className="blocks" key={type}>
        ...
        <div className="block-list">
          {Object.entries(icons).map((icon) => (
            <button key={icon[0]} tabIndex="0" onClick={this.changeBlock} className={`block-item${icon[0] === blockName ? ' is-active' : ''}`} block-type={type} block-name={icon[0]}>
              {icon[1]}
            </button>
          ))}
        </div>
  ...
  viewModeRenderer() {
    ...
    return viewList.map((v, k) => (
      <button key={k} className={`device${view === v.name ? ' is-active' : ''}`} data-view={v.name} onClick={this.changeView}>
        {v.icon}
      </button>
    ))
  }
  ...
  render() {
    const { darkMode, theme, blockName, blockType, sidebar, view, currentKeyCode } = this.state
    return (
